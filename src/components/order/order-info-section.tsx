...
export function OrderInfoSection({ invoice, actions }: OrderInfoSectionProps) {
  ...
  const items = [
    ...
    { label: "Balance", value: `${invoice.CurrencyRef.name} ${invoice.Balance.toFixed(2)}` },
    { label: "Total Amount", value: `${invoice.CurrencyRef.name} ${invoice.TotalAmt.toFixed(2)}` },
    { label: "Tax", value: `${invoice.CurrencyRef.name} ${invoice.TxnTaxDetail.TotalTax.toFixed(2)}` },
